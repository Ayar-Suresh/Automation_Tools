...
output_json_file = "anime_data.json"
# ----------------------------
...
def nearest_quality(q):
    """Convert any quality to nearest standard (480p, 720p, 1080p)"""
    if isinstance(q, str):
        q = re.sub(r"[^0-9]", "", q)
    try:
        q = int(q)
...
async def main():
    ...
    total_episodes = sum(len(anime["episodes"]) for anime in anime_data.values())
    logger.info(f"Processing complete! Found {total_anime} anime with {total_episodes} episodes")

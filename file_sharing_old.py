...
# Source Channels (Where files come from - Fallback only)
SOURCE_CHANNELS = [
    -1003175788400, 
    -1003498912074   
]
...
# QUEUE 1: The Traffic Control (Sending)
request_queue = asyncio.Queue()

# QUEUE 2: The Janitor List (Deleting)
pending_deletions = []
...
async def worker_distributor():
    ...
    while True:
        ...
        # 2. Process batch SEQUENTIALLY (Fix for Random Order)
        # Note: We removed asyncio.gather to prevent out-of-order delivery
        for req in batch:
            await process_single_request(req)
            # Tiny sleep to ensure Telegram servers process order correctly
            await asyncio.sleep(0.05) 
        
        # 3. Wait to refill limits
...
async def worker_janitor():
    ...
    while True:
        try:
            ...
            # 2. Update the main list to keep ONLY non-expired items
            pending_deletions = [x for x in pending_deletions if x['delete_at'] > now]
            
            # 3. Delete the expired messages
            for item in expired:
                try:
            ...
            # 4. Sleep for 10 seconds before checking again
...
async def process_single_request(req):
    ...
    # Try channels in the EXACT order provided
    for channel_id in channel_list:
        try:
            ...
            if sent_msg: break # Success! Stop trying other channels.
        except errors.FloodWaitError as e:
            # If flood wait, pause then retry this specific message
            await asyncio.sleep(e.seconds)
            # Retry logic could be added here, but for now we sleep and skip to next channel/msg
...
def find_batch_in_season(season_data, batch_key):
    ...
    if target_key.startswith("special"):
        specials = []
        def extract_specials(d):
            if isinstance(d, dict):
                if "specials" in d: specials.extend(d["specials"].values())
                for v in d.values(): extract_specials(v)
        extract_specials(season_data)
        specials.sort(key=lambda x: x.get('name', ''))
        return specials
    return None
...
async def batch_handler(event):
    ...
    try:
        ...
        for item in files:
            ...
            # ✅ Order maintained (no set(), no random mixing)
